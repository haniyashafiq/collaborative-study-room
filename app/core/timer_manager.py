...
class TimerManager:
    ...
    The manager calls a callback every second with (room_id, remaining, is_running).
    """
    ...
    async def start(self, room_id: int, duration: int, broadcast_cb: Callable[[int, int, bool], asyncio.Future]):
        """
        Start or restart a timer for a room.
        broadcast_cb(room_id, remaining, is_running) will be awaited every second.
        """
        async with self._lock:
            ...
            task = asyncio.create_task(self._run(room_id, broadcast_cb))
            self.tasks[room_id] = task

    async def _run(self, room_id: int, broadcast_cb: Callable[[int, int, bool], asyncio.Future]):
        try:
            while True:
                async with self._lock:
                    info = self.timers.get(room_id)
                    if not info or not info.is_running:
                        # send final state
                        await broadcast_cb(room_id, info.remaining if info else 0, False)
                        break
                    if info.remaining <= 0:
                        info.is_running = False
                        await broadcast_cb(room_id, 0, False)
                        break
                ...
                async with self._lock:
                    ...
                    # broadcast update
                    await broadcast_cb(room_id, info.remaining, True)
        except asyncio.CancelledError:
            # broadcast stopped state
            async with self._lock:
                info = self.timers.get(room_id)
            if info:
                await broadcast_cb(room_id, info.remaining, info.is_running)
            raise
        finally:
            # cleanup task entry
            async with self._lock:
    ...
    async def resume(self, room_id: int, broadcast_cb: Callable[[int, int, bool], asyncio.Future]):
        async with self._lock:
            info = self.timers.get(room_id)
            if not info:
                return
    ...
    async def reset(self, room_id: int, duration: Optional[int], broadcast_cb: Callable[[int, int, bool], asyncio.Future]):
        async with self._lock:
